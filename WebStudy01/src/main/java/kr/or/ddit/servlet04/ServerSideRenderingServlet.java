...
public class ServerSideRenderingServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		html.append("<html>                  ");
		html.append("<body>                  ");
		html.append(String.format("<h4>서버의 시간대 : %s</h4>", information));
		html.append("</body>                 ");
		html.append("</html>                 ");
		
		resp.setContentType("text/html;charset=utf-8");
		try(
			PrintWriter out = resp.getWriter();		// try()안에 넣어서 자동으로 close()됨 (ctrl + 마우스 왼쪽 눌러서 closable 상속받고 있으면 close()자동으로 가능한 것)
		){
			out.print(html);
		}
	}
