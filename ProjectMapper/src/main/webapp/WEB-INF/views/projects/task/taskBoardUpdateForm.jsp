...
<div id="project_container" data-context-path="${pageContext.request.contextPath}" data-afi="${taskBoard.taskAfi}" data-task-id="${taskBoard.taskId}">
	...
	<form id="taskForm" class="mb-9 dropzone dropzone-multiple" data-dropzone="data-dropzone" method="post" enctype="multipart/form-data"
			action="${pageContext.request.contextPath}/project/taskBoard/update/${taskBoard.taskId}">
		<div class="row">
			<div class="col-md-8">
				<input type="hidden" name="taskParentId" value="${taskBoard.taskParentId}" />
				<div class="col-md-12">
					<input type="text" class="form-control" name="taskName" placeholder="제목" value="${taskBoard.taskName}" required>
				</div>
				<div class="mb-6">
					<textarea class="tinymce" name="taskContent" data-tinymce='{"height":"15rem", "placeholder":"내용"}'>${taskBoard.taskContent}</textarea>
				</div>
				<!-- 멀티 첨부 파일 형식 -->
			...
			<div class="col-md-4">
				      ...
				      <div class="row gx-3">
				        <div class="col-12 col-sm-6 col-xl-12">
				          <div class="mb-4">
				            ...
				            <input type="date" value="${taskBoard.taskStartDate}" class="form-control" name="taskStartDate" />
				          </div>
				        </div>
				        <div class="col-12 col-sm-6 col-xl-12">
				          <div class="mb-4">
							...
							<input type="date" value="${taskBoard.taskEndDate}" class="form-control" name="taskEndDate">
				          </div>
				        </div>
				        <div class="col-12 col-sm-6 col-xl-12">
				          	<div class="d-flex flex-wrap mb-2">
							...
							<select name="taskAssigneeId" class="form-select" required>
								<option value="${taskBoard.taskAssigneeId}" selected>${taskBoard.userName}</option>
				        ...
				        <div class="col-12 col-sm-6 col-xl-12">
				          <div class="mb-0">
				            <div class="d-flex flex-wrap mb-2">
				              <h6 class="mb-0 text-body-highlight me-2">진행도</h6>
				            </div>
							<input type="number" class="form-control taskDetail" name="taskProgress" value="${taskBoard.taskProgress}" required>
				          </div>
				        </div>
				      </div>
				    </div>
				...
				<div class="d-flex flex-wrap flex-row-reverse">
					<a href="javascript:window.history.back()">
						<button type="button" class="btn btn-primary" id="task_removeBtn" style="cursor:pointer;">목록</button>
					</a>
					<span>&nbsp;</span>
					<button type="submit" class="btn btn-primary" style="cursor:pointer;">저장</button>
				</div>
				</div>
			</div>
		</div>
...
<script type="module" src="${pageContext.request.contextPath}/resources/js/project/projectTaskUpdate.js"></script>